...
export function StoryHome({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`${styles.site} ${className}`} ref={page}>
      ...
      <footer className={styles.footer}><div><Link href="/" className={styles.logoLink} aria-label="ConnectAble.work home"><Wordmark /></Link><p>Every ability. Every possibility.</p></div><p className={styles.footerCredit}>An Inclusion Revolution project.<br />Made for a more connected world.</p><nav aria-label="Footer navigation"><Link href="/about">About</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><a href="#main" aria-label="Back to top"><ArrowRight size={19} className={styles.backToTop} /></a></nav></footer>
